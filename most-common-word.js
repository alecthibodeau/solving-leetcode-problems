 ...
 * Run locally: $ node most-common-word.js
 */
...
var mostCommonWord = function(paragraph, banned) {
  const spaceAndSymbolsRegex = /[ !?',;.]+/;
  const wordsNotBanned = paragraph.toLowerCase().split(spaceAndSymbolsRegex).filter(word => !banned.includes(word) && word !== '');
  const wordsFrequency = {};
  let mostFrequentWord;
  let maximumFrequency = 0;

  wordsNotBanned.forEach(word => {
    wordsFrequency[word] = (wordsFrequency[word] || 0) + 1;
  });

  for (const word in wordsFrequency) {
    if (wordsFrequency[word] > maximumFrequency) {
      mostFrequentWord = word;
      maximumFrequency = wordsFrequency[word];
    }
  }
